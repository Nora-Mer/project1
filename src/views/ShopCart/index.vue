  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        ...
        <div class="sumbtn">
          <a class="sum-btn" @click="tradeInfo">结算</a>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type,disNum,cart){
        // type:为了区分这三个元素
        // disNum形参: +1 -1 input最终的个数（并不是变化量）
        // cart:哪一个产品（skuId skuNum）
        // 向服务器发请求，修改数量
        switch(type){
          case 'add':
            disNum = 1
          ...
          case 'minus':
            ...
            break
          case 'change':
            // 用户输入进来的最终的量是非法的（不是数字）|| 出现负数，那么带给服务器的还是原来的数字
      ...
      updataChecked(cart,event){
        try {
          // 如果修改成功，再次获取服务器数据【购物车列表】
          // 带给服务器的参数isChecked不是布尔值，而是 0 1
          let checked = event.target.checked ? '1' : '0'
          this.$store.dispatch('updataCheckedById',{skuId:cart.skuId,isChecked:checked})
          // 再次发送请求
          this.getData()
        } catch (error) {
          // 如果失败，返回失败信息
