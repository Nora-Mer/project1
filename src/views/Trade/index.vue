...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapGetters(['detailArrayList','totalAmount','totalNum']),
      ...mapState({
      ...
      // 将来提交订单最终选择的地址
      userDefaultAddress(){
        // find查找数组当中符合条件的元素返回最终结果
        // 注意，这里报错的，可能是userAddress还没获取到值，就被用来find了，所以这里要先给mapstate后面加一个||{} userDefaultAddress--{}
        return this.userAddress.find(item => item.isDefault==1) || {} //返回的是对象
      }
    },
    methods:{
      // 修改地址
      // userAddress:数组 user：数组里的每一项
      ...
      async goPay(){  
        ...
        // 其余六个参数
        let data = {
          "consignee": this.userAddress.consignee, //最终收件人的名字
      ...
      // 发请求reqSubmitTrade 需要带两个参数tradeNo,data
